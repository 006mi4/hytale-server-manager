...
export const BansTab: React.FC<BansTabProps> = ({ serverId }) => {
  ...
  const handleAdd = async () => {
    ...
    setAdding(true);
    try {
      await window.electronAPI.servers.sendCommand(serverId, `/ban add ${name}`);
  ...
  const handleRemove = async (name: string) => {
    await window.electronAPI.servers.sendCommand(serverId, `/ban remove ${name}`);
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={listStyle}>
        {entries.length === 0 ? (
          <div style={emptyStyle}>No bans</div>
        ) : (
          entries.map(name => (
